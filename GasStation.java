...
/**
 ...
 Return the starting gas station's index if you can travel around the circuit once in
...
public class GasStation {
    ...
    public int canCompleteCircuit(int[] gas, int[] cost) {
        ...
        int N = gas.length;
        int guess = 0;
        int currentStartTotal = 0;
        int sum = 0;

        for(int i = 0; i < N; ++i){
            currentStartTotal += (gas[i] - cost[i]);
            sum += (gas[i] - cost[i]);

            if(currentStartTotal < 0){
                currentStartTotal = 0;
                guess = (i + 1) % N;
            }
        }
